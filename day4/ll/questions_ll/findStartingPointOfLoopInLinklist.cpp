...
public:
    ...
    Node *findStartingNodeOfLoopInLinklist(Node *head)
    {
        Node *fast = head;
        Node *slow = head;
        while (fast != NULL && fast->next != NULL)
...
int main()
{
    ...
    tailNode->next = head->next->next->next;
    Node *startingNodeOfLoop = obj.findStartingNodeOfLoopInLinklist(head);
    cout << startingNodeOfLoop->next << " " << startingNodeOfLoop->data << endl;
    return 0;
}